...
from amyloid_wrappers.core.fasta import read_fasta
...
# Predictors whose external runner accepts multiple sequences in one FASTA file.
BATCH_CAPABLE_RUNNERS = frozenset({"appnn"})
...
def chunk_items(items: list[tuple[str, str]], batch_size: int) -> list[list[tuple[str, str]]]:
    if batch_size < 1:
...
def run_multifasta_pipeline(
    ...
    *,
    ...
    requested = _normalise_predictors(predictors)
    ...
    for runner_key in runner_keys:
        _run_runner_batches(
            ...
            items,
            layout,
            batch_size=effective_batch if runner_key in BATCH_CAPABLE_RUNNERS else 1,
...
def _normalise_predictors(predictors: Iterable[str] | None) -> list[str]:
    ...
    for batch_index, batch in enumerate(batches, start=1):
        ...
        for protein_id in batch_ids:
            ...
            result = runner.run(
                ...
                raw_csv=raw_csv if runner_key == "appnn" else None,
            )
...
def _run_path_batch(
    ...
    *,
    ...
    if skip_run:
        ...
        results_csv = batch_work / runner.results_filename
        if not results_csv.is_file():
            alt = batch_work / f"{batch_fasta.stem}_{runner.results_filename}"
            results_csv = alt if alt.is_file() else results_csv
        if not results_csv.is_file() and save_raw_files is not None:
            archived = _find_archived_raw(save_raw_files, protein_ids[0], "path")
            if archived is not None:
                emit(f"[PATH] batch {batch_label}: loaded raw from {archived}")
                results_csv = archived
        if not results_csv.is_file():
            raise FileNotFoundError(
                f"[PATH] batch {batch_label}: expected {results_csv} or archived raw "
                f"under {save_raw_files} (--skip-run)"
            )
    else:
...
def _cleanup_cache_dir(config_path: str | None, *, emit: LogFn) -> None:
    from amyloid_wrappers.core.cache import clear_cache_dir
