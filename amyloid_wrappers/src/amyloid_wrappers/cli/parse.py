"""``amyloid-parse`` — convert raw predictor output to standard CSV."""
...
predictors and raw inputs:
  ...
  waltz       WALTZ .dat tab file (position<TAB>score) + FASTA
  pasta       one energy value per line + FASTA
...
examples:
  ...
  amyloid-parse crossbeta --input result.json --fasta protein.fasta -o out.csv

  python -m amyloid_wrappers parse waltz --input APP.dat --fasta APP.fasta -o out.csv
